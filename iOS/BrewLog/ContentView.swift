...
struct ContentView: View {
    @EnvironmentObject var store: BrewLogStore
    @EnvironmentObject var purchases: PurchaseManager
    @State private var showAddSheet = false
    @State private var showSettings = false
    @State private var showPaywall = false
    @State private var editingEntry: BeerEntry?
    ...
    @ViewBuilder
    private func row(for entry: BeerEntry) -> some View {
        HStack {
...
struct EntryEditorView: View {
    ...
    let existing: BeerEntry?
    let onSave: (BeerEntry) -> Void

    init(entry: BeerEntry?, onSave: @escaping (BeerEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        ...
        NavigationStack {
            Form {
                Section("Beer") {
                    ...
                    TextField("Style / Brewery", text: $detail)
                        .accessibilityIdentifier("entryDetailField")
                    DatePicker("Tried On", selection: $date, displayedComponents: .date)
                        .accessibilityIdentifier("entryDatePicker")
                    Toggle("Favorite", isOn: $isFavorite)
                        .accessibilityIdentifier("entryFavoriteToggle")
                }
            }
            .navigationTitle(existing == nil ? "Add Beer" : "Edit Beer")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = existing ?? BeerEntry(name: name, detail: detail, date: date)
                        entry.name = name
                        entry.detail = detail
                        entry.date = date
